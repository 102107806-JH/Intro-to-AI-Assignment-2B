...
class TrafficFlowDataSet(Dataset):
    def __init__(self, data_set_file_name, sequence_length, selected_scats_site):
        self._transform_dict = None  # Dictionary that is used to store transforms. Given to the models so that the transforms can be accessed easily later #

        self._sequence_length = sequence_length  # The sequence length (number of time steps) #
        ...
        self._data_set_index_to_data_array_index_populator(data_list, sequence_length)  # Constructs a dictionary which allows you to enter an index which will then be converted into the dataset index #

        np_data_array = np.asarray(data_list, dtype=np.float32)  # Convert the list into a numpy array #

        self._np_data_array = self._transform_data(np_data_array)  # Apply normalisation transformations #

    def __len__(self):  # The length of the dataset
        return self._length
    ...
    def _put_data_set_in_list(self, data_set_file_name, selected_scats_site):
        """
        Takes the dataset file name and a scats site and then puts the corresponding data into a list.
        :param data_set_file_name: The file name of the dataset to be loaded in.
        :param selected_scats_site: The scats site that you want to select can be set to 'ALL' to capture
        all data
        :return: A list that contains the dataset. Invalid or illegal data points have been remove as well.
        """
    ...
    def _data_set_index_to_data_array_index_populator(self, data_list, sequence_length):
        ...
        need to be excluded.
        :param data_list: The data list that contains the dataset
        :param sequence_length: The length number of time steps in the sequence
        :return: Null
        """
    ...
    def _transform_data(self, np_data_array):
        ...
        # Setting the transform dictionary
